...
class MotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            'GT_FOLDER': os.path.join(code_path, 'data/gt/moranget/'),  # Location of GT data
            'TRACKERS_FOLDER': os.path.join(code_path, 'data/trackers/moranget/'),  # Trackers location
            'OUTPUT_FOLDER': None,  # Where to save eval results (if None, same as TRACKERS_FOLDER)
            'TRACKERS_TO_EVAL': None,  # Filenames of trackers to eval (if None, all in folder)
            'CLASSES_TO_EVAL': ['orange'],  # Valid: ['orange']
            'BENCHMARK': 'moranget',  # Valid: 'MOT17', 'MOT16', 'MOT20', 'MOT15'
            ...
            'SEQMAP_FILE': "data/gt/moranget/seqmaps/moranget-test.txt",  # Directly specify seqmap file (if none use seqmap_folder/benchmark-split_to_eval)
    ...
    def __init__(self, config=None):
        ...
        # Get classes to eval
        self.valid_classes = ['orange']
        self.class_list = [cls.lower() if cls.lower() in self.valid_classes else None
                           for cls in self.config['CLASSES_TO_EVAL']]
        if not all(self.class_list):
            raise TrackEvalException('Attempted to evaluate an invalid class. Only orange class is valid.')
        self.class_name_to_class_id = {'orange': 1}
